    ...
    func testArticleDesignMetadataAndConditionalHighlighting() throws {
        ...
        for article in Article.all {
            try app.test(.GET, article.canonicalPath) { response in
                ...
                XCTAssertEqual(sponsorLinks.count, 1)
                let sponsorLink = try XCTUnwrap(sponsorLinks.first as? XMLElement)
                XCTAssertEqual(sponsorLink.attribute(forName: "target")?.stringValue, "_blank")
                XCTAssertEqual(Set((sponsorLink.attribute(forName: "rel")?.stringValue ?? "").split(separator: " ")), ["sponsored", "noopener"])
                XCTAssertTrue(sponsorLink.attribute(forName: "aria-label")?.stringValue?.contains("opens in a new tab") == true)
